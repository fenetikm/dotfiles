return {
  ...
  },
  {
    ...
    keys = {
      ...
      { ']x',         '<cmd>lua require("trouble").next({ jump = true })<cr>', silent = true, noremap = true, desc = "Next item (Trouble)" },
      { '[x',         '<cmd>lua require("trouble").prev({ jump = true })<cr>', silent = true, noremap = true, desc = "Prev item (Trouble)" },
    },
    opts = {
      signs = {
        error = '!',
