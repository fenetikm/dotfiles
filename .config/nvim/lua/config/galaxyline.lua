...
-- current method for other langs
...
local current_method = function()
  if not table.contains({'php'}, vim.bo.filetype) then
    return ''
  end
  local vmethod = vim.api.nvim_exec([[
...
table.insert(gls.left, {
  CurrentMethod = {
    provider = current_method,
    highlight = {colors.method, colors.status_2},
  }
})
